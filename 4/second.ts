...
const overlaps = (fSection: Section, sSection: Section) => fSection[0] < sSection[1] && fSection[1] >= sSection[0];

const doesOverlap = ([fSection, sSection]: Pair): number => {
    if (fullContain(fSection, sSection) || fullContain(sSection, fSection)) return 1;
    if (overlaps(fSection, sSection) || overlaps(sSection, fSection)) return 1;
    return 0;
}
